								  ...
								  BOOL bApply /* =TRUE */)
{
	...
	if (bApply && ::IsWindow(GetSafeHwnd())) {
		...
		if (BITSET(dwAdd,StyleUnderline) || BITSET(dwRemove,StyleUnderline))
			SwitchFont();
		if (BITSET(dwAdd,StyleItalic) || BITSET(dwRemove,StyleItalic))
			SwitchFont();
...
void CHyperLink::SetURL(CString strURL,CString strTip)
{
	m_strURL = strURL;
...
{	
	...
	if (pFont != NULL) {
		pFont->GetLogFont(&lf);		
		lf.lfUnderline = BITSET(m_dwStyle,StyleUnderline);
		lf.lfItalic=BITSET(m_dwStyle,StyleItalic);
		m_Font.DeleteObject();
		m_Font.CreateFontIndirect(&lf);
		SetFont(&m_Font);					
	}	
...
void CHyperLink::SetDefaultCursor()
{
	...
	{
        //g_hLinkCursor=AfxGetApp()->LoadCursor( IDC_HAND );
    ...
    if ((UINT)result <= HINSTANCE_ERROR) {		
        ...
        if (GetRegKey(HKEY_CLASSES_ROOT, _T(".htm"), key) == ERROR_SUCCESS) {
            ...
            if (GetRegKey(HKEY_CLASSES_ROOT,key,key) == ERROR_SUCCESS) {
                ...
                lstrcat(pos, _T(" "));
                lstrcat(pos, url);
                result = (HINSTANCE) WinExec( lm_u2a_s(key),showcmd);
            }
        }
	}
...
{	
	...
	{
        MessageBeep(MB_ICONEXCLAMATION);	// 不能跳到链接
  	    CString str = "不能打开链接:\n\n" + ReportError(result);
...
//激活提示Activate
void CHyperLink::ActivateToolTip(BOOL Activate)
{
