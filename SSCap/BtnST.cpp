...
//
DWORD CButtonST::SetIcon(int nIconIn, int nCxDesiredIn, int nCyDesiredIn,
	int nIconOut, int nCxDesiredOut, int nCyDesiredOut,
	int nIconDisable, int nCxDesiredDisable, int nCyDesiredDisable
	)
{
	HICON		hIconIn	    = NULL;
	HICON		hIconOut		= NULL;
	...
	case NULL:
		break;
	case (int)BTNST_AUTO_GRAY:
		hIconDisable = BTNST_AUTO_GRAY;
		break;
	case (int)BTNST_AUTO_DARKER:
		hIconDisable = BTNST_AUTO_DARKER;
		break;
	default:
		hIconDisable = (HICON)::LoadImage(hInstResource, MAKEINTRESOURCE(nIconDisable), IMAGE_ICON, nCxDesiredDisable, nCyDesiredDisable, 0);
		break;
	} // swit
...
//			Failed loading the specified resource.
//
DWORD CButtonST::SetIcon(int nIconIn, int nIconOut, int nDisable )
{
	return SetIcon(nIconIn, 0, 0, nIconOut, 0, 0,nDisable, 0 , 0 );
} // End of SetIcon
...
//			Failed loading the specified resource.
//
DWORD CButtonST::SetIcon(HICON hIconIn, HICON hIconOut, HICON hDisable)
{
	BOOL		bRetValue;
	ICONINFO	ii;
	...
	if (hIconIn)
	{
		...
		if (hDisable)
		{
			...
			{
			case (int)BTNST_AUTO_GRAY:
				hDisable = CreateGrayscaleIcon(hIconIn);
				break;
			case (int)BTNST_AUTO_DARKER:
				hDisable = CreateDarkerIcon(hIconIn);
				break;
			} // switch

			m_csIcons[2].hIcon = hDisable;
			// Get icon dimension
			::ZeroMemory(&ii, sizeof(ICONINFO));
			bRetValue = ::GetIconInfo(hDisable, &ii);
			if (bRetValue == FALSE)
			{
				FreeResources();
				return BTNST_INVALIDRESOURCE;
