...
void BCMenuData::SetWideString(const wchar_t *szWideString)
{
	...
	if (szWideString)
    {
		...
		if (m_szMenuText)
			wcscpy_s(m_szMenuText,sizeof(wchar_t)*(wcslen(szWideString)), szWideString);
    }
	else
...
void BCMenu::DrawItem_Win9xNT2000 (LPDRAWITEMSTRUCT lpDIS)
{
	...
	if(state & MF_SEPARATOR){
		...
		if(checkflag||standardflag||selectedflag||disableflag){
			...
			if(checkflag && checkmaps && checkmaps->GetSafeHandle() ){
				...
				CPoint ptImage(rect.left+2,rect.top+2+dy);
				if(bitmap && bitmap->GetSafeHandle() )bitmap->Draw(pDC,xoffset,ptImage,ILD_TRANSPARENT);
			}
		}
...
BOOL BCMenu::GetBitmapFromImageList(CDC* pDC,CImageList *imglist,int nIndex,CBitmap &bmp)
{
	...
	if( imglist && imglist->GetSafeHandle() )
		hIcon = imglist->ExtractIcon(nIndex);
	...
	dc.FillSolidRect( 0,0, m_iconX, m_iconY ,m_newclrBack );
...
bool BCMenu::AppendMenu (BCMenu* pMenuToAdd, bool add_separator /*= true*/, int num_items_to_remove_at_end /*= 0*/)
{
	...
	for (int iLoop = 0; iLoop < iMenuAddItemCount; iLoop++ )
	{
		...
		if (!pSubMenu)
		{
			...
			if (!AppendMenu( nState, nItemID, (LPTSTR)(LPCTSTR)sMenuAddString )){
		...
		else
		{
			...
			BCMenu* pNewPopupMenu = AppendODPopupMenu ((LPTSTR)(LPCTSTR)sMenuAddString);
